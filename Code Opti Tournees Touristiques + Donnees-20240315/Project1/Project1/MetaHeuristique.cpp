...
void MetaHeuristique::SolutionGenetique() {
	...
	for (int i_Jour = 0; i_Jour < instance->get_Nombre_Jour(); i_Jour++) {
		...
		i_FO += i_Score_Jour;
		pp_Meilleure_Sequence_par_Jour[i_Jour] = pair<float, vector<int>>(i_Score_Jour, pi_Sequence);
	}
...
void MetaHeuristique::Initialisation() {
	...
	for (int i_Jour = 0; i_Jour < instance->get_Nombre_Jour(); i_Jour++) {
		...
		p_Meilleure_Sequence = Heuristique_v1::CalculMeilleureJournee(instance, ppi_POI_par_Jour[i_Jour], map_Score_POI, pii_Hotels_par_Jour[i_Jour].first, pii_Hotels_par_Jour[i_Jour].second, i_Jour, {});
...
vector<pair<float, vector<int>>> MetaHeuristique::GenerationNSequence(vector<int> pi_POI, int i_Nombre_de_Liste_A_Construire, int i_Jour) {
	vector<pair<float, vector<int>>> pi_NSequences = {};
	...
	for (int i_N = 0; i_N < i_Nombre_de_Liste_A_Construire; i_N++) {
		...
		pair<float, vector<int>> pi_Sequence(0.0, {});
		for (int i_POI : pi_POI_Ordre_Aleatoire)
		{
			pi_Sequence = Heuristique_v1::MeilleureSequence(pi_Sequence, i_POI, instance, pii_Hotels_par_Jour[i_Jour].first, pii_Hotels_par_Jour[i_Jour].second, i_Jour);
		}


		printf("liste => {");
		for (int idx = 0; idx < pi_Sequence.second.size(); idx++) {
			printf(" %d ,", pi_Sequence.second[idx]);
		}
		printf("}\n");
		...
		int i_Score_Sequence = 0;
		for (int i_POI : pi_Sequence.second){
			i_Score_Sequence += instance->get_POI_Score(i_POI);
		}
	...
	vector<pair<float, int>> vec_Sequences_Triees(map_Score_Sequence.begin(), map_Score_Sequence.end());
	...
	vector<pair<float, vector<int>>> pi_NSequences_Triees;
